...
void TimeBroadcast(int i, char *model)//传入参数为秒数 
{
	...
	system("del D:\\Documents\\2.VBS");//删除临时语音文件
	return;
}
...
void GetDayWords(int day)
{
	...
	char out[100];
	int d = 0;
	FILE *p = fopen("D:\\Documents\\TBTwords.txt", "r");
	if (p == NULL)
	{
...
Start:
	...
	for (i = 0; i<3600; i++)
	{
		...
		if (TBT->day_words == 1)
		{
			...
			GetDayWords(YearDay);
			/*wordsFp = fopen("D:\\Documents\\TBTwords.txt", "r");
			if (wordsFp == NULL)
			{
			printf("每日一言文件打开失败\n");
			//system("pause");
		...
		if (TBT->all_reset == 1)
		{
			...
			system("pause");
			Initial_TBT_Set("D:\\Documents\\TBTset.ini");
			system("pause");
			goto Start;
		}
		if (TBT->time_reset == 1)
